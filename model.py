...
class MSA_AUGMENTOR(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.shared = nn.Embedding(config.vocab_size, config.d_model) # 共享一个嵌入空间？
        self.debug=config.debug
        ...
        self.encoder = T5Stack(encoder_config, self.shared) # encoder和decoder都用T5Stack表示
    ...
    def set_input_embeddings(self, new_embeddings): # 输入嵌入
        self.shared = new_embeddings
        self.encoder.set_input_embeddings(new_embeddings)
        self.decoder.set_input_embeddings(new_embeddings)

    def set_output_embeddings(self, new_embeddings): # 输出嵌入
        self.lm_head = new_embeddings

    def get_output_embeddings(self): # 线性层？
        return self.lm_head
